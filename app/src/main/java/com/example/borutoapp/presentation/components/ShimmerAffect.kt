...
fun ShimmerAffect() {
    LazyColumn(
        contentPadding = PaddingValues(all = SMALL_PADDING), verticalArrangement = Arrangement.spacedBy(SMALL_PADDING)
    ) {
        items(count = 2) {
            AnimatedShimerITEM()
        }
...
@Composable
fun AnimatedShimerITEM() {
    val transient = rememberInfiniteTransition()
    val alphaAnimation = transient.animateFloat(
        initialValue = 1f,
        targetValue = 0f,
        animationSpec = infiniteRepeatable(animation = tween(durationMillis = 500, easing = FastOutLinearInEasing), repeatMode = RepeatMode.Reverse)
    )
    ShimmerAffectItem(alphaAnimation.value)
}

@Composable
fun ShimmerAffectItem(alpha: Float) {
    Surface(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Column(modifier = Modifier.padding(all = MEDIUM_PADDING), verticalArrangement = Arrangement.Bottom) {
            Surface(
                modifier = Modifier
                    .alpha(alpha = alpha)
                    .fillMaxWidth(0.5f)
...
@Composable
@Preview
fun shimerITemPreview() {
    AnimatedShimerITEM()
}

@Composable
@Preview(uiMode = Configuration.UI_MODE_NIGHT_YES)
fun shimerITemDArkPreview() {
    AnimatedShimerITEM()
}